...
import { fetchProfileById, sendConnectionRequest } from "@/actions";
...
export default function ProfilePage() {
  ...
  if (loading) {
    return <div className="max-w-4xl mx-auto py-10">Loading profile...</div>;
  }

  if (error) {
    return <div className="max-w-4xl mx-auto py-10 text-red-500">{error}</div>;
  }

  if (!profileData) {
    return <div className="max-w-4xl mx-auto py-10">Profile not found</div>;
  }
  ...
  const handleConnect = async (receiverId: string) => {
    try {
      ...
      //   setProfileData(prev => prev ? {...prev, isConnected: true} : null);
      // }

      const request = await sendConnectionRequest(receiverId)
    } catch (error) {
    ...
    switch (user.role) {
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      <div className="text-center p-2 bg-muted rounded-lg">
                        <div className="text-lg font-bold">{user.stats.goals}</div>
                        <div className="text-xs text-muted-foreground">Goals</div>
                      </div>
                      <div className="text-center p-2 bg-muted rounded-lg">
                        <div className="text-lg font-bold">{user.stats.assists}</div>
                        <div className="text-xs text-muted-foreground">Assists</div>
                      </div>
                      <div className="text-center p-2 bg-muted rounded-lg">
                        <div className="text-lg font-bold">{user.stats.matches}</div>
      ...
      case Role.SCOUT:
        return (
          <Card className="overflow-hidden shadow-sm hover:shadow-md transition-shadow rounded-xl">
            <CardContent className="p-6">
              <h2 className="text-lg font-semibold mb-4">Scout Details</h2>
              <div className="space-y-4">
                {user.bio && (
                  <p className="text-sm text-muted-foreground">{user.bio}</p>
                )}
  ...
  return (
    <div className="max-w-4xl mx-auto pb-10">
      ...
      <div className="relative mb-20 sm:mb-24">
        ...
        <div className="absolute right-4 sm:right-8 -bottom-12 sm:-bottom-16 flex gap-2">
          {isOwnProfile ? (
            <Button onClick={() => setShowEditProfile(true)} className="rounded-full">
              <Edit className="h-4 w-4 mr-2" />
              Edit Profile
            </Button>
          ) : (
            <>
              {isConnected ? (
                ...
                <Button onClick={() => handleConnect(userId)} className="rounded-full">
                  <UserPlus className="h-4 w-4 mr-2" />
                 {sendingRequest ? "sending request": "Connect"}
                </Button>
              )}
            </>
          )}
              ...
              {user.posts.length === 0 ? (
                <Card className="overflow-hidden shadow-sm rounded-xl">
                  <CardContent className="p-8 text-center">
                    <p className="text-muted-foreground">No posts yet</p>
                    {isOwnProfile && (
                      <Button className="mt-4 rounded-full" onClick={() => setShowCreatePost(true)}>
                        <Plus className="h-4 w-4 mr-2" />
                        Create Your First Post
                      </Button>
                    )}
